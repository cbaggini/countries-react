	...
	return (
		...
		{country && 
			...
			<figure className="detailFigure">
				<img className="detailFlag" src={country.flag} alt={country.name}></img>
			</figure>
			<article className="mainInfo">
				...
				<div className="detailContainer">
					<div>
						...
						<p><strong>Population: </strong>{country.population.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</p>
					...
					<div>
						...
						<p><strong>Currencies: </strong>{country.currencies.map(el => el.name).join(' - ')}</p>
						<p><strong>Languages: </strong>{country.languages.map(el => el.name).join(', ')}</p>
					</div>
				</div>
				<p className="borders">
